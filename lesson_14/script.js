...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Обработчик клика по кнопке Deposit (Пополнение)
depositButton.onclick = function () {
    const accountId = parseInt(accountIdInput.value); // Получаем ID аккаунта
    const amount = parseFloat(amountInput.value); // Получаем сумму

    // Проверка, чтобы ID аккаунта и сумма были корректными
    if (isNaN(accountId) || isNaN(amount) || amount <= 0) {
        alert("Пожалуйста, введите корректный ID аккаунта и сумму");
        return;
    }

    // Ищем аккаунт по введенному ID
    const account = bank.find(acc => acc.accountNumber === accountId);
    if (account) {
        account.deposit(amount); // Вызываем метод пополнения на найденном аккаунте
    } else {
...
// Обработчик клика по кнопке Withdraw (Снятие)
withdrawButton.onclick = function () {
    ...
    const account = bank.find(acc => acc.accountNumber === accountId);
    if (account) {
        account.withdraw(amount); // Вызываем метод снятия средств на найденном аккаунте
    } else {
        alert("Аккаунт не найден");
    }
...
// возможный вариант решения задачи с кнопками  withdraw и deposit
...
// еще один возможный вариант решения задачи с кнопками  withdraw и deposit
...
document.addEventListener('DOMContentLoaded', () => {
    const deleteButton = document.getElementById('deleteAccount'); // Кнопка удаления
    const accountIdInput = document.getElementById('accountId'); // Поле ввода для ID аккаунта

    // Обработчик клика по кнопке Delete (Удалить)
    deleteButton.onclick = function () {
        const accountId = parseInt(accountIdInput.value); // Получаем ID аккаунта

        // Проверка, чтобы ID аккаунта был корректным
        if (isNaN(accountId) || accountId <= 0) {
            alert("Пожалуйста, введите корректный ID аккаунта");
            return;
        }

        // Ищем аккаунт по введенному ID
        const accountIndex = bank.findIndex(acc => acc.accountNumber === accountId);
        
        if (accountIndex !== -1) {
            // Удаляем аккаунт из массива bank
